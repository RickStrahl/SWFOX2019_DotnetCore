...
***            Southwestfoxdemo.ini and you can customize operation there.
***
***            [ServerConfig]
***            Virtual=Southwestfoxdemo            
***            ScriptMaps=wc,wcs,wcsx,swf    
...
***
***    Assume: Called from SouthwestfoxdemoMain.prg startup code
***            with lcAction parameter:
***            DO SouthwestfoxdemoMain with "CONFIG"
***
***            Or launch your Exe from the Windows Command Line:
***            Southwestfoxdemo.exe config
***
***            or with a specific IIS site/virtual
***            Southwestfoxdemo.exe config "IIS://localhost/w3svc/12/root"
***
...
IF FILE("WCONNECT.APP")
   MessageBox("This feature only works in the full version of Web Connection",48,"Southwestfoxdemo")
   RETURN
ENDIF
...
IF !IsAdmin() 
   MESSAGEBOX("Admin privileges are required to configure the IIS Web Server." + CHR(13) +;
              ...
              48,"Southwestfoxdemo Server Configuration")
   RETURN
ENDIF

*** Try to read from Southwestfoxdemo.ini [ServerConfig] section
loApi = CREATEOBJECT("wwAPI")
lcIniPath = FULLPATH("Southwestfoxdemo.ini")
...
IF FILE("Southwestfoxdemo.exe")
   RUN /n4 "Southwestfoxdemo.exe" /regserver

   *** Optionally set DCOM permission - only set if needed
   *** requires that DComLaunchPermissions.exe is available in .\tools\
   * DCOMLaunchPermissions("Southwestfoxdemo.SouthwestfoxdemoServer","INTERACTIVE")
   * DCOMLaunchPermissions("Southwestfoxdemo.SouthwestfoxdemoServer","username","password")
ENDIF
